...
## فاز اول
برای طراحی برنامه، ما اول کلاس main را به پوشه graph اضافه کردیم و آنرا پیاده‌سازی کردیم تا بتواند ورودی را از کاربر دریافت کند و در ابتدا یک پاسخ بدیهی به آن بدهد تا بعدا متدهای لازم پیاده سازی شوند. پس از آن به سراغ طراحی سه دستور اول رفتیم که در آن از الگوی state استفاده کردیم. در ابتدا یک interface با نام [state](Pattern/src/main/java/graph/State.java) ایجاد کردیم متدهای لازم برای سه دستور اول را در آن قرار دادیم. پس از آن دو کلاس با نام ‌های [OneWayState](Pattern/src/main/java/graph/OneWayState.java) و [TwoWayState](Pattern/src/main/java/graph/TwoWayState.java) را که از کلاس state ارث‌بری می‌کنند را پیاده‌سازی کردیم بدون اینکه متدهای داخل آنرا پیاده سازی کنیم. در نهایت برای مدیریت راه ها یک کلاس [RoadControl](Pattern/src/main/java/graph/RoadControl.java) ایجاد کردیم که دارای یک متغیر state است که می‌تواند بین دو حالت یک طرف و دو طرفه تغییر کند. این کلاس بدون در نظر گرفتن نوع state توابع آن را فراخونی خواهد کرد و بدین شکل از الگو state استفاده می‌کنیم. در نهایت پس از ایجاد متدهای لازم با بدنه خالی، تست‌های لازم را در فایل [StateTest](Pattern/src/test/java/graph/StateTest.java) ایجاد می‌کنیم و مرحله به مرحله با روش TDD متدهای لازم را پیاده‌سازی می‌کنیم. بعد از آن سراغ سناریو‌های بعدی می‌رویم که در پیاده‌سازی آنها از الگو strategy استفاده می‌کنیم. برای پیاده‌سازی‌های دو روش مختلف استراتژی اتوبوس و قطار، دو کلاس [TrainStrategy](Pattern/src/main/java/graph/TrainStrategy.java) و [BusStrategy](Pattern/src/main/java/graph/BusStrategy.java) را ایجاد می‌کنیم که هر دو از کلاس [Strategy](Pattern/src/main/java/graph/Strategy.java) ارث‌بری میکنند و متدهای آنرا با توحه به روش خود پیاده‌سازی خواهند کرد. در ابتدا تنها متدهای با بدنه خالی را قرار می‌دهیم و بعد تست‌ها را در [StrategyTest](Pattern/src/test/java/graph/StrategyTest.java) ایجاد می‌کنیم و مشابه قبل،‌مرحله به مرحله با روش TDD متدها را پیاده‌سازی می‌کنیم.
در نهایت کلاس [Main](Pattern/src/main/java/graph/Main.java) را نهایی می‌کنیم با در نطر گرفتن فایل‌های جدید آن را پیاده‌سازی می‌کنیم تا دستورات مورد نیاز را اجرا کند. برای اجرا برنامه می‌توانید متد main از این کلاس را اجرا کنید. نحوه ورودی داده در ترمینال توضیح داده می‌شود.
...
### دو تغییر دیگر
برای تغییر اول، کلاس Log را طبق الگوی Singleton بازآرایی میکنیم زیرا از استاتیک استفاده میکرد و برای مورد دوم، در کلاس Parser  متود خیلی طولانی را با الگوی extract method به تعدادی متود کوچکتر تجزیه می‌کنیم.
